...
_TOKEN_LIFE_TIME = datetime.timedelta(seconds=3600)
...
class QivivoAuth(object):
    ...
    def access_token(self):
        if self._accessToken is None or self._tokenLimit < datetime.datetime.now():  # Token should be renewed
            data = {'grant_type': 'client_credentials',
                    'client_id': self._clientId,
                    'client_secret': self._clientSecret,
                    'scope': self._scope
                    }
            resp = QivivoRequest.post(url=_AUTH_URL, data=data)
            self._accessToken = resp['access_token']
            self._tokenLimit = datetime.datetime.now() + _TOKEN_LIFE_TIME
...
class QivivoData(object):
    ...
    def __init__(self, auth):
        self.getAuthToken = auth.access_token
    ...
    def headers(self):
        return {'Content-Type': 'application/json',
                'Authorization': 'Bearer {}'.format(self.getAuthToken)}

    def get_devices(self):
        url = _BASE_URL + 'devices'
        response = QivivoRequest.get(url=url, headers=self.headers)
    ...
    def set_thermostat_temperature(self, th_id, temp, dur):
        ...
        response = QivivoRequest.post(url=url, headers=self.headers, data=payload)
        return response['message']

    @check_serial
    def del_thermostat_temperature(self, th_id):
        url = _BASE_URL + 'devices/thermostats/{}/temperature/temporary-instruction'.format(th_id)
        response = QivivoRequest.delete(url=url, headers=self.headers)
        return response['message']
    ...
    def set_thermostat_active_program(self, th_id, prog_id):
        ...
        response = QivivoRequest.put(url, headers=self.headers)
        return response['message']
...
class QivivoRequest(object):
    """
        Send Qivivo requests and process response to json
    """
